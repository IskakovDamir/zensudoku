    ...
    <div className="min-h-screen bg-white dark:bg-zinc-950 text-zinc-900 dark:text-zinc-100 flex flex-col">
      <header className="flex items-center justify-between px-6 py-4 border-b border-zinc-200 dark:border-zinc-800">
        ...
        <div className="flex items-center gap-2">
          <ThemeToggle />
          <Link href="/auth/signin" className="text-sm text-zinc-500 dark:text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-100 transition-colors px-3 py-1.5">
            Sign in
          </Link>
          <Link href="/pro" className="flex items-center gap-1 text-sm font-medium px-3 py-1.5 rounded-lg bg-zinc-100 dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full max-w-2xl mb-16">
          {DIFFICULTIES.map((d) => (
            <Link
              key={d.key}
              href={`/game/${d.key}`}
              className={`group flex items-center justify-between p-5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900/50 ${d.hover} transition-all duration-200 hover:shadow-sm`}
            >
...
function DailyButton() {
  return (
    <Link
      href="/game/daily"
      className="relative inline-flex items-center gap-2 px-7 py-3.5 rounded-xl bg-zinc-900 dark:bg-zinc-100 text-zinc-100 dark:text-zinc-900 text-sm font-semibold transition-all duration-200 hover:bg-zinc-800 dark:hover:bg-zinc-200 before:absolute before:inset-0 before:rounded-xl before:ring-2 before:ring-orange-500/40 before:animate-pulse"
    >
