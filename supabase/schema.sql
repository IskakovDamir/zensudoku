...
create policy "Only service role can insert daily challenges"
  on public.daily_challenges for insert with check (auth.role() = 'service_role');

create index daily_challenges_date_idx on public.daily_challenges(date desc);

create or replace view public.leaderboard as
  select s.id, s.user_id, u.email, u.city, s.difficulty, s.time_seconds, s.mistakes, s.completed_at,
    rank() over (partition by s.difficulty order by s.time_seconds asc, s.mistakes asc) as rank
  from public.scores s join public.users u on u.id = s.user_id;
